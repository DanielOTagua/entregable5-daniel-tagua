  ...
  return (
    ...
    >
      ...
      <section className="poke__card-body">
        ...
        <ul className="poke__card-types-container">
          {pokemon?.types.map((type) => (
            <li className="poke__card-type" key={type.type.name}>
              {type.type.name}
            </li>
          ))}
        </ul>
      ...
      <footer className="poke__card-footer">
        <ul className="poke__card-stats-container">
          {pokemon?.stats.map((stat) => (
            <li className="poke__card-stat" key={stat.stat.name}>
              <span className="poke__card-label">{stat.stat.name}</span>
              <span className={`poke__card-number color-${pokemon?.types[0].type.name}`}>{stat.base_stat}</span>
            </li>
          ))}
        </ul>
      </footer>
